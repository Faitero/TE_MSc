...
#
# and produce all the tlasification clasification used on this study:
#
...
#panPan1<- read.table("Alu_exons/All_Aluexons_3SS_hg19_Distance_hg38_panPan1.tab", sep="\t", header = TRUE)
...
#papHam1<- read.table("Alu_exons/All_Aluexons_3SS_hg19_Distance_hg19_papHam1.tab", sep="\t",  header = TRUE)
...
## Some testing on higher ans lower values
...
## Write LONG table
write.table(whole_final, file = "whole_final.LONG.tab", sep = "\t", na = "NA", row.names = FALSE, col.names = TRUE)


## Same than prvious table reading but this time in WIDE format
 ## Asign column names
...
## Import Jans definitions of splice type exons (Constitutive, Alternative .... Cryptic...and NAs!!)
...
write.table(total_furthest, file = "whole_final_5sp.WIDE.RICH.tab.tab", sep = "\t", na = "NA", row.names = FALSE, col.names = TRUE)
